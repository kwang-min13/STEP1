...
def load_ab_test_results(filepath: str = 'logs/ab_test_results.csv') -> pd.DataFrame:
    """
    A/B 테스트 결과 로드
...
def calculate_basic_stats(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    stats = {
        ...
        'group_a_avg_satisfaction': group_a['satisfaction'].mean(),
        'group_b_avg_satisfaction': group_b['satisfaction'].mean(),
...
def t_test_satisfaction(df: pd.DataFrame) -> Dict[str, Any]:
    """
    T-검정으로 만족도 차이 검증
    ...
    Returns:
        검정 결과 딕셔너리
    """
    group_a_satisfaction = df[df['group'] == 'A']['satisfaction']
    group_b_satisfaction = df[df['group'] == 'B']['satisfaction']
    
    t_stat, p_value = ttest_ind(group_a_satisfaction, group_b_satisfaction)
    
    return {
        'test_name': 'Independent T-Test (Satisfaction)',
        't_statistic': t_stat,
        'p_value': p_value,
        'significant': p_value < 0.05,
        'group_a_mean': group_a_satisfaction.mean(),
        'group_b_mean': group_b_satisfaction.mean(),
        'difference': group_b_satisfaction.mean() - group_a_satisfaction.mean()
    }
...
def analyze_ab_test(filepath: str = 'logs/ab_test_results.csv') -> Dict[str, Any]:
    """
    전체 A/B 테스트 분석 수행
    ...
    t_test_sat = t_test_satisfaction(df)
    t_test_pur = t_test_purchases(df)
    
    return {
        'basic_stats': basic_stats,
        'chi_square_test': chi2_result,
        't_test_satisfaction': t_test_sat,
...
def print_analysis_summary(results: Dict[str, Any]) -> None:
    ...
    print(f"\n[*] Satisfaction:")
    print(f"  Group A: {stats['group_a_avg_satisfaction']:.2f}/5")
    print(f"  Group B: {stats['group_b_avg_satisfaction']:.2f}/5")
    ...
    print(f"\n[*] T-Test (Satisfaction):")
    print(f"  t-statistic: {t_sat['t_statistic']:.4f}")
    print(f"  p-value: {t_sat['p_value']:.4f}")
    print(f"  Significant: {'[YES]' if t_sat['significant'] else '[NO]'} (alpha=0.05)")
