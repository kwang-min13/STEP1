    ...
    def create_user_features(self, 
        ...
        WITH recent_transactions AS (
            SELECT 
                ...
                article_id,
                price,
                EXTRACT(HOUR FROM CAST(t_dat AS TIMESTAMP)) as purchase_hour
        ...
        SELECT 
            us.customer_id,
            ROUND(us.avg_purchase_hour, 2) as avg_purchase_hour,
            us.purchase_count,
            us.unique_items,
        ...
        stats = con.execute("""
            SELECT 
                ...
                AVG(avg_purchase_hour) as avg_hour,
        ...
        logger.info(f"평균 구매 시간대: {stats[2]:.2f}시")
        logger.info(f"구매 빈도 분포:")
        logger.info(f"  - High: {stats[3]:,} ({stats[3]/stats[0]*100:.1f}%)")
        logger.info(f"  - Medium: {stats[4]:,} ({stats[4]/stats[0]*100:.1f}%)")
        logger.info(f"  - Low: {stats[5]:,} ({stats[5]/stats[0]*100:.1f}%)")
        logger.info("=" * 60)
