...
def calculate_sample_size_for_ab_test(
    ...
    # 만족도에 대한 표본 크기 (예시)
    # 가정: 만족도 평균 3.5, 표준편차 1.0
    baseline_satisfaction = 3.5
    satisfaction_std = 1.0
    satisfaction_effect = baseline_satisfaction * target_lift
    
    satisfaction_result = calculate_sample_size_mean(
        baseline_mean=baseline_satisfaction,
        baseline_std=satisfaction_std,
        minimum_detectable_effect=satisfaction_effect,
        alpha=alpha,
        power=power
    )
    ...
    max_sample_size = max(
        ...
        satisfaction_result['sample_size_per_group']
    )
    ...
    return {
        ...
        "satisfaction_analysis": satisfaction_result,
        "parameters": {
...
def print_sample_size_report(result: Dict[str, Any]):
    ...
    sat = result['satisfaction_analysis']
    print(f"\n3. User Satisfaction Score")
    print(f"   Baseline Mean: {sat['baseline_mean']:.2f}")
    print(f"   Expected Test Mean: {sat['expected_test_mean']:.2f}")
    print(f"   Minimum Detectable Difference: {sat['minimum_detectable_effect']:.2f}")
    print(f"   Effect Size (Cohen's d): {sat['effect_size_cohen_d']:.3f}")
    print(f"   Required Sample Size: {sat['sample_size_per_group']:,} per group")
