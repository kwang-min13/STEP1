...
def run_ab_test(num_users: int = 20, mode: str = "fast", llm: Optional[int] = None, seed: int = 42):
    ...
    """
    logger.info("=" * 70)
    logger.info(f"A/B 테스트 시뮬레이션 시작 (users={num_users}, mode={mode}, seed={seed})")
    logger.info("=" * 70)
    ...
    try:
        ...
        # candidate_gen은 rec_service 내부에서 생성되므로 별도 생성 불필요
        ...
        for i, user_id in enumerate(user_ids, 1):
            ...
            try:
                if group == 'A':
                    ...
                    result = ab_simulator.simulate_group_b(user_id, vu)
                
                t2 = time.time()
                
                logger.info(f"클릭 여부: {result['clicked']}")
                logger.info(f"발송 시간: {result['send_time']}시")
                logger.info(f"추천 상품 수: {result['num_items']}개")
                logger.info(f"[timing] persona={t1-t0:.2f}s sim={t2-t1:.2f}s")
        ...
        if results:
            ...
            csv_path = output_path / 'ab_test_results.csv'
        ...
        # 리소스 정리 (반드시 실행)
        logger.info("\n리소스 정리 중...")
        if ab_simulator is not None:
            try:
                ab_simulator.close()
                logger.info("✓ ABTestSimulator 종료")
            except Exception as e:
                logger.error(f"ABTestSimulator 종료 실패: {e}")
        
        if rec_service is not None:
            try:
                rec_service.close()
                logger.info("✓ RecommendationService 종료")
            except Exception as e:
                logger.error(f"RecommendationService 종료 실패: {e}")
        
        logger.info("✓ 시뮬레이션 종료 완료")
...
def run_simulation(num_users: int = 5, mode: str = "fast", llm: Optional[int] = None, seed: int = 42):
    ...
    try:
        ...
        for i, real_user_id in enumerate(real_user_ids, 1):
            ...
            logger.info(f"만족도: {evaluation.get('satisfaction')}/5")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", type=str, default="fast", choices=["fast", "full"], help="실행 모드")
